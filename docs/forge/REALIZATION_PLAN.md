...
| L1 · Geometry model + inspect | K23, M25, AR22b, AR32.4, AR32.5 | **built + reviewed, gate pending (AFK)**; M25 reached the CLI as `bpt inspect` on 2026-08-29 — until then it was a library function the help text already advertised |
...
| L6 · Independent validation harness + restore drill | K31, D1, M30, AR27, B5 | **built** (2026-08-29): every geometry validates through the real binary, the sabotage test catches an ambiguous puzzle, the restore drill runs from a fresh clone in CI, B5 stamps the revision. D1's sweep runs in CI at 100 puzzles up to 14x14, fewer above, and skips 18x18 and 20x20 — it prints every reduction, so a green run never reads as full coverage. The reason is Q7 |
| L7 · Parallelism, benchmarks, baselines | M22, K30, AR25, B4 | **built** (2026-08-29): M22 generates on all cores and produces byte-identical batches to a single-threaded run (16x16, 8 puzzles: 25.8 s on one core, 8.2 s on sixteen); K30's criterion benchmarks and the G8 threshold test exist; AR25's baseline is recorded in `benchmarks/baseline.json` and guarded at 1.5x p95. B4 remains Kenny's "later". **G8's "any special type under 30 s" is not met**: 9x6x6 measures 94 s |
| L8 · Invented types + property testing | K28, D4, M28, M24 | **built** (2026-08-29): 4x10x10 and 8in12in16 generate and solve end to end from geometry files alone (D4); M28's property tests cover uniqueness, subset-of-solution, reproducible grading and regeneration over random geometries; M24 adds `--clues` and `--symmetry`. K28's tag mini-round is still open, so invented types emit no tag and pass their geometry alongside |
